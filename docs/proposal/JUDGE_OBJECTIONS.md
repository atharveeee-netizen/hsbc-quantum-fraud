...
1. **Geometric Convergence:** Centered Kernel Alignment (CKA) between the Projected Quantum Kernel and the tuned Classical RBF kernel was **0.5741** (and 0.9373 for Fidelity). The angle-embedding feature map closely mirrors the metric topology of classical Gaussian RBF kernels rather than uncovering an orthogonal, classically intractable geometry.
2. **Sample Variance & Statistical Insignificance:** While PQK achieved a nominal PR-AUC of 0.5540 vs 0.6561 for Classical RBF ($\Delta = -0.1021$), a 1,000-resample paired bootstrap test revealed a 95% confidence interval of $[-0.0383, +0.1821]$ with $p = 0.246$ (Bonferroni $p = 0.492$). The confidence interval encompasses zero, confirming that the observed delta is indistinguishable from sampling noise.
