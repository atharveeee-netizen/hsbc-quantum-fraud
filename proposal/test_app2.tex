...
Figure~\ref{fig:shap} depicts the TreeSHAP feature importance attributions across test transactions (=118,108$). Feature attributions show distinct row-level variance without constant-attribution artifacts.
\vspace{-4pt}
\begin{figure}[h!]
\centering
\includegraphics[width=0.78\linewidth,height=4.2cm,keepaspectratio]{feature_attribution_shap.png}
\vspace{-4pt}
\caption{\textbf{TreeSHAP Global Summary Plot (IEEE-CIS Benchmark).} Demonstrates feature attributions across =118,108$.}
\label{fig:shap}
\end{figure}
\vspace{-6pt}

\subsection*{C.3 Cryptographic Provenance \& Verification of the 5 Submission Artifacts}
