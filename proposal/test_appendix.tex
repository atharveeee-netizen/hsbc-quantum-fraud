...
\subsection*{B.5 TreeSHAP Local and Global Feature Attribution}
Figure~\ref{fig:shap} illustrates the TreeSHAP summary attribution across the out-of-sample test partition. Features exhibit row-level variance confirming non-trivial multi-feature risk discrimination.
\vspace{-4pt}
\begin{figure}[h!]
\centering
\includegraphics[width=0.72\linewidth,height=3.8cm,keepaspectratio]{feature_attribution_shap.png}
\vspace{-4pt}
\caption{\textbf{TreeSHAP Global Feature Attribution Summary on IEEE-CIS Benchmark.} Attributions vary per transaction across =118,108$, confirming absence of constant-feature artifacts.}
\label{fig:shap}
\end{figure}
\vspace{-6pt}
